...
operatoren = ["a", "every", "no", "not every"]  # auffaellig ist: es wird jeweils nur eine form benötigt im englischen

#subjekte = ["graf", "fremde", "blick", "kirche", "schloss", "bild", "auge", "dorf", "turm", "bauer", "weg", "gast", "tag", "haus", "tisch", "knecht"]
subjekte = ["earl", "stranger", "gaze", "church", "castle", "picture", "eye", "village", "tower", "peasant", "path", "guest", "day", "house", "table", "farmhand"]
...
def elementarSatz(operatoren, subjekte, praedikate):
    ...
    elementarSatz = operator1 + " " + subjekt1 + " is " + praedikat1 + random.choice(satzOperatoren) + operator2 + " " + subjekt2 + " is " + praedikat2 + "."
    return elementarSatz.upper()

try:
    while 1:
        print elementarSatz(operatoren, subjekte, praedikate)
        sleep(1)
